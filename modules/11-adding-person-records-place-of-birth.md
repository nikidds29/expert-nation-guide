...
A place should really be a fixed, shared list: one record for
Ballarat, reused by everyone born there. Before creating a new Place
record, check **Select item** to see whether the place you need
already exists, and reuse it if it does, rather than typing "Ballarat"
in again as a new record.
...
The **Select item** tab returns a results list of existing records, in
the same Title format used throughout this guide (names blurred above,
as these are real records).
...
the **Mapping** tab elsewhere in the system. It's worth filling these
in when you have the coordinates to hand.
