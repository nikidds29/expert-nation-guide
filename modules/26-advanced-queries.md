---
layout: default
title: Advanced Queries
nav_order: 26
---

# Advanced Queries

Advanced search is useful in this collection for narrowing by
**class** or by **resource template**, but can also be used for keyword or
value search.

## Where to find it

From the main Items screen, click **Advanced search**, next to the
page count near the top.

![Items screen showing 1-25 of 144011 items, with the Advanced search link highlighted, real record titles and owner names blurred](../assets/images/advanced-queries-01-items-list.png)

## Searching by class

**Search by class** narrows results to every item carrying a
particular Class, drawn from the vocabularies in use across the
collection (Dublin Core, Custom Expert Nation, schema.org, and so
on), regardless of which resource template created them.

![Advanced search screen with the Search by class dropdown open, listing classes grouped by vocabulary](../assets/images/advanced-queries-02-search-by-class.png)

## Searching by resource template

**Search by template** narrows results to items created with one
specific resource template (Person, Associated person, Life event,
and the rest), listed here grouped by the account that created each
one.

![Advanced search screen with the Search by template dropdown open, listing resource templates grouped by account, account names and emails blurred](../assets/images/advanced-queries-03-search-by-template.png)

For the full label-to-property lookup table, see the
[Quick reference guide](28-quick-reference-guide.md).
