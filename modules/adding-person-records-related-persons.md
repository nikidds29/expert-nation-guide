...
record for cases that don't need a full profile). This guide only
covers building out the full Person template, so Associated person gets
its own pass once every Person field is covered. For now, just know
one of the two has to exist before you link to it.
